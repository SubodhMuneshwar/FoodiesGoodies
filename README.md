...
[![Tests](https://img.shields.io/badge/Tests-41%20Passed-brightgreen?logo=checkmarx&logoColor=white)](FoodiesGoodies.Tests/)
...
### 2. AI Dietician & Nutritional Intelligence
- Direct nutritional analysis powered by Edamam Nutrition API.
- Instant caloric breakdown, macronutrient ratios (protein, fats, carbs), micronutrient percentages (vitamins, minerals).
- Automated health label detection (e.g., Dairy-Free, Low-Sodium, Sugar-Conscious).
...
## 🧪 Automated Testing Suite (41 Tests)

The test suite in **`FoodiesGoodies.Tests/`** verifies system correctness, security policies, and validation boundaries.
...
- Mocks outbound network calls (`MockHttpMessageHandler` for Edamam and `IContactService` for SMTP).
...
### Test Coverage Breakdown (41 Total Tests):

```
Passed!  - Failed: 0, Passed: 41, Skipped: 0, Total: 41, Duration: 5 s
```
